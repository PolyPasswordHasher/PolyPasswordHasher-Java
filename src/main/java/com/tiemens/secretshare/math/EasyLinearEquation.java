 ...
 *
 *
 * Example: given 3 equations like: 1491 = 83a + 32b + 22c 329 = 5a + 13b + 22c
 * 122 = 3a + 2b + 19c The goal is to solve for a, b, and c [3 unknowns, 3
 * equations].
 *
 * The problem above is encoded into a matrix of numbers like: 1491 83 32 22 329
 * 5 13 22 122 3 2 19 and stored in this class as a List[Row] objects.
 *
 * Then, this class can "solve" it into the 'diagonal 1', giving: 11 1 0 0 16 0
 * 1 0 3 0 0 1 Which in turn means that a=11, b=16 and c=3
 *
 *
 * This implementation is called "easy" because it is really straight-forward,
 * but also really inefficient. Values are "canceled" by multiplying the two
 * together. e.g. "8" and "4" just needs the "4" to be multiplied by 2, then
 * subtracted. This implementation takes 8*4 and subtracts 4*8 because that just
 * works, and you don't need to compute the least common multiple.
 *
 * This implementation is also "easy" since it doesn't use any lin-eq library.
 * There are a lot of those libraries available: it turns out it was easier to
 * write this class than to figure out how to use the horrible APIs they
 * presented. [The ones with good APIs didn't support BigInteger]
 *
 * @author tiemens
 *
 */
public final class EasyLinearEquation {
  ...
  private static Logger logger = Logger.getLogger(EasyLinearEquation.class.
          getName());
   ...
   *
   * Polynomial linear equations are a special case, because the C,x,x^2,x^3
   * coefficients can be turned into the rows we need by being given: a) which
   * "x" values were used b) what "constant" values were computed This
   * information happens to be exactly what the holder of a "Secret" in
   * "Secret Sharing" has been given. So this constructor can be used to
   * recover the secret if given enough of the secrets.
   *
  ...
  /**
   * Convenience factory to create an instance with 'int's instead of
   * BigIntegers.
   *
   ...
   *
   * @param inMatrix given in BigIntegers, where the first column is the
   * constant and all the other columns are the variables
   * @return instance
   */
  ...
  public EasySolve solve() {
    ...
    for (int workrowindex = 0, maxindex = solverows.size(); workrowindex
            < maxindex; workrowindex++) {
      ...
      for (int fixindex = workrowindex + 1; fixindex < maxindex; fixindex++) {
        ...
        Row cancelrowr = solverows.get(fixindex).cancelColumn(
                columnIndexToCancel,
                otherrow,
                modulus);
    ...
    for (int workrowindex = solverows.size() - 1; workrowindex >= 0;
            workrowindex--) {
      Row reducedToOne = solverows.get(workrowindex).solveThisRow(modulus);
      logger.fine("reverse, index=" + workrowindex + " is " + reducedToOne.
              debugRow());
      ...
      debugRows("After reverse loopindex=" + workrowindex + " finished",
              solverows, modulus);
    }
        //
    // the matrix should look like this now:
    ...
    public BigInteger getAnswer(int i) {
      ...
      if (i == 0) {
        throw new SecretShareException(
                "Answer index 0 is the constant, not an answer."
  ...
  private static class Trial {
    ...
    public BigInteger getResult() {
      if (correct) {
        ...
        throw new SecretShareException(
                "Tried to get result from non-correct trial");
      }
    }
...
//            }
    ...
    public static Trial pickSuccess(List<Trial> list) {
      if (list.get(list.size() - 1).correct) {
        ...
        throw new SecretShareException(
                "Programmer error, no trial correct, list.size=" + list.size());
      }
    }
    ...
    public static List<Trial> createList2(final BigInteger original,
      ...
      list.add(new Trial("mod both", original.mod(useModulus), divideby.mod(
              useModulus)));
      ...
      if ((gcd != null)
        ...
        list.add(new Trial("gcd both", divO.mod(useModulus), divD.
                mod(useModulus)));
      }
      return list;
    }
  ...
  private static class Row {
    ...
    /**
     * @return row with col[0] non-zero and one-and-only-one other column
     * non-zero, all the other columns must be ZERO OR throw exception
     * @throws SecretShareException if more than 2 columns [the 1st and 1
     * other] are non-zero
     */
    public Row solveThisRow(final BigInteger useModulus) {
      ...
      for (int col = 0, n = ret.cols.length; col < n; col++) {
        if ((col == 0)
          ...
          BigInteger result = divideNormallyOrModulus(original, divideby,
                  useModulus);
    ...
    /**
     * This _should be_ the implementation. The problem is, it doesn't work.
     *
     */
    @SuppressWarnings("unused")
    ...
    /**
     * The modulus stuff is really strange. Sometimes the divide-by just
     * works. Sometimes it is negative but "odd", and needs mod() to
     * positive and "even". Sometimes it is positive but "too big and odd"
     * and needs mod() to a smaller "even" number. Sometimes it needs
     * multiple-add-the-modulus, especially for small values of the modulus
     * versus large values of the coefficients.
     *
     * This routine just tries a bunch of things, including: 1) original /
     * divideby and 2) (original mod useModulus) / divideby See class Trial.
     *
     * @param original
     * @param divideby
     * @param useModulus
