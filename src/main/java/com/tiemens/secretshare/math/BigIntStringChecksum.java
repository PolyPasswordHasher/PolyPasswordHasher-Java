...
/**
 * Data structure to support a "URI-like" string that allows BigInteger values
 * to be encoded as a hex string with a checksum.
 *
 * Syntax: bigintcs:hhhhhh-hhhhhh-CCCCCC Example:
 * bigintcs:bd2c52-b16d74-d51456-d0f89a-30c932-b2f6c1-3a9ce3-7b4387-0F2CA0
 *
 * Note: negative BigIntegers are supported. This adds a "-" at the front, after
 * the "bigintcs:" e.g. -100 is the string 'bigintcs:-000064-BBC6EC' The
 * checksum takes the "-" into account, therefore, 'bigintcs:-000064-BBC6EC' =
 * -100 'bigintcs:000064-BBC6EC' = error
 *
 * The "hhhhhh-" section is repeated as many times as needed. The first
 * "hhhhhh-" section is 0-padded as needed. The first section is either "hhhhhh"
 * or "-hhhhhh" [dash means negative BigInteger].
 *
 * The case of the hex digits "a-f" versus "A-F" does not matter.
 *
 * Convention is that the hhhhhh- section is lower case, the CCCCCC- section is
 * upper case.
 *
 * Also, the checksum is COMPUTED using all lower-case hhhhhh- sections, even if
 * the input is not given that way.
 *
 */
public final class BigIntStringChecksum {
  ...
  /**
   * The prefix string that identifies the 6hex-6hex-md5sum6hex pattern we've
   * invented to signify a BigIntegerChecksum value. This value must be in
   * lower case.
   */
  ...
  // ==================================================
  /**
   * Contains a hex-encoded [i.e. RADIX 16] string of the big integer. Does
   * not contain dashes ("-")
   */
  private final String asHex;

  /**
   * Contains a hex-encoded string of the md5sum of "asHex". Our
   * implementation is limited to 6 characters of checksum.
   */
  ...
  // ==================================================
  /**
   * Utility to test if the input string can even -possibly- be a
   * BigIntStringChecksum encoded. This is check is "necessary, but not
   * sufficient" for the input string to parse correctly.
   *
   * @param input the string to check
   * @return true if the string starts with (case-insensitive)
   * BigIntStringChecksum.PREFIX_BIGINT_DASH_CHECKSUM false all other cases
   */
  ...
  /**
   * Take input string, and create the instance. As of version 1.3.2 forward,
   * the case of the INPUT is not relevant. Note, however, that when the
   * Checksum CCCCCC is computed, the hex hhhhhh- string is LOWER CASE.
   *
   * @param bics string in "bigintcs:hhhhhh-hhhhhh-CCCCCC" format
   * @return big integer string checksum object
   * @throws SecretShareException on error, such as null input, OR input
   * doesn't start with correct prefix OR string does not have 0-9a-f digits
   ...
   *
   * @param bics string in "bigintcs:hhhhhh-hhhhhh-CCCCCC" format
   * @return big integer string checksum object OR null if incorrect format,
   * error parsing, etc.
   */
  ...
  /**
   * Routine to construct an instance that allows you to print the hex
   * strings. Such as String s = BigIntStringChecksum
   * create(biginteger).toString(); s.equals("bigintcs:00f3ea-CBA3D0");
   *
  ...
  // ==================================================
  /**
   * Construct an instance of BISC. Performs NO validation of input.
   *
  ...
  /*
   * default
   */ BigIntStringChecksum(final String inAsHex,
  ...
  /**
   * Return the original BigInteger. (or throw an exception if something went
   * wrong).
   *
  ...
  private static byte[] computeMd5ChecksumFull(String inAsHex2) {
    ...
    if (testonlyUseInternalMd5Impl) {
      ...
      md5summer = Md5ChecksummerFactory
              .createFromClassName(
                      "com.tiemens.secretshare.md5sum.Md5ChecksummerImpl");
    }

    byte[] bytes = md5summer.
            createMd5Checksum(inAsHex2.toLowerCase().getBytes());
